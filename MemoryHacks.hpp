...
bool MemHackInfCarryWeight(DWORD pId, HANDLE pHandle) {
    uintptr_t gameDllBaseAddr = GetModuleBaseAddress(pId, (char*)"Game.dll"); //Gets  the base address of our needed process/module (Only tested this with .dll files not exe)
    ...
    uintptr_t addr = FindDMAAddy(pHandle, readAddr, currentCarryWeightOffsets); //This is getting calculating our final address by calculating through our currentCarryWeightOffsets vector (the addy this returns is what we right our new data too)
    if (addr == readAddr) {
        return FALSE;
    }
...
bool MemHackLevelUp(DWORD pId, HANDLE pHandle) {
    uintptr_t gameDllBaseAddr = GetModuleBaseAddress(pId, (char*)"Game.dll");
    if (gameDllBaseAddr == 0) {
        return FALSE;
    }
    ...
    uintptr_t addr = FindDMAAddy(pHandle, readAddr, currentExperienceOffsets);
    if (addr == readAddr) {
        return FALSE;
    }
